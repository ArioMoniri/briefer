...
HELP = (
    ...
    "*Force a sheet* — add `add to articles` or `add to events` (also "
    "`as an article`, `to events sheet`) to your message to override "
    "auto-detection. Each queued item has a *✖ Cancel* button while it waits.\n\n"
    "*What I accept*\n"
    ...
    "*Add your own note to a row*\n"
    "Put a line `note: <your description>` in the message (or the file/photo "
    "caption) you send. It's filed into the row's *Notes* column instead of "
    "being analysed, and accumulates across re-sends.\n\n"
    "*Tags dropdown*\n"
    "The *My Tags* column offers a dropdown of every tag already in use — pick "
    "one or type a new one.\n\n"
    "*Reply to any result to act on that row*\n"
    "Reply to one of my result messages with `pass this to John` (assign), "
    "`remind me tomorrow` (reminder), or `note: …` — it applies to *that* row "
    "instead of being analysed as new content. Names are case-insensitive.\n\n"
